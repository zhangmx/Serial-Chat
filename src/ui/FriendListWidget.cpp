...
void FriendListWidget::setPortManager(SerialPortManager* manager)
{
    ...
    if (m_portManager) {
        QObject::connect(m_portManager, &SerialPortManager::friendListChanged,
                         this, &FriendListWidget::refreshList);
        QObject::connect(m_portManager, &SerialPortManager::userStatusChanged,
                         this, [this](const QString& portName, PortStatus status) {
...
void FriendListWidget::onItemClicked(const QString& portName)
{
    selectPort(portName);
}
...
void FriendListWidget::onAddButtonClicked()
{
    emit addPortRequested();
}

void FriendListWidget::onGroupButtonClicked()
{
...
void FriendListWidget::setupHeader()
{
    ...
    m_searchEdit->setStyleSheet("QLineEdit { border: 1px solid #E0E0E0; border-radius: 15px; padding: 5px 15px; background-color: white; }");
